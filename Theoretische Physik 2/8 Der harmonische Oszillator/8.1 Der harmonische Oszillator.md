...
Wir erkennen also, dass jeder Eigenvektor zum EW $n+1$/$\hbar \omega(n+ 3/2)$ proportional zu $a^{\dagger}\ket{n}$. Sie sind also kolinear, und damit nicht linear unabhängig für beliebige $c_{i}$, weshalb folgt dass jeder Eigenvektor nicht entartet ist. Und das ist gut. Wir haben also auch eine Formel jeden weiteren Eigenzustand ausgehend vom Grundzustand $\ket{0}$ zu basteln:

$$
\ket{n}=c_{n}(a^{\dagger})^{n}\ket{0}  =
$$